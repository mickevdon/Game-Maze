...
float FXOS8700CQ::get_pitch_angle(){
   ...
   Data value = get_values(); // get values into struct
    float tan_p = -value.ax/(sqrt(value.ay*value.ay + value.az*value.az)); //formula from datasheets
    float p = atan(tan_p);//formula from datasheet
    return p*180/3.14; //return pitch angle in degrees
}

float FXOS8700CQ::get_roll_angle(){
    Data value = get_values();  // get values into struct
    float tan_r = value.ay/value.az; //formula from datasheet
    float r = atan(tan_r); //formula from datasheet
    return r*180/3.14;//return roll angle
}
